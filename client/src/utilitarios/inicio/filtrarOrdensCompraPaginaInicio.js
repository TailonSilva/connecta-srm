export function filtrarOrdensCompraPaginaInicio(ordensCompra, filtros, produtos, opcoes = {}) {
  if (!Array.isArray(ordensCompra) || ordensCompra.length === 0) {
    return [];
  }
  ...
  const obterData = typeof opcoes?.obterData === 'function'
    ? opcoes.obterData
    : (ordemCompra) => ordemCompra?.[campoData];
  ...
  return ordensCompra.filter((ordemCompra) => {
    const atendeData = validarPeriodo(obterData(ordemCompra), filtros?.[nomeInicio], filtros?.[nomeFim], exigirData);
    ...
    const atendeComprador = idsComprador.length === 0 || idsComprador.includes(String(ordemCompra?.idComprador || ''));
    const itens = Array.isArray(ordemCompra?.itens) ? ordemCompra.itens : [];
