export const TOTAL_COLUNAS_GRID_COTACOES = 100;
const BASE_LEGADA_COLUNAS_GRID_COTACOES = 24;
const MAX_SPAN_COLUNA_OCULTA = 19;
const mapaRotulosLegadosColunasGridCotacoes = new Map([
...
export const colunasGridCotacoes = [
  {
    id: 'codigo',
    rotulo: 'Codigo',
...
const idsPermitidos = new Set(colunasGridCotacoes.map((coluna) => coluna.id));
const mapaColunasGridCotacoes = new Map(
  colunasGridCotacoes.map((coluna) => [coluna.id, coluna])
);

export function normalizarConfiguracoesColunasGridCotacoes(valor) {
  ...
  listaNormalizada.forEach((item, indice) => {
    ...
    const colunaBase = mapaColunasGridCotacoes.get(id);
  ...
  const configuracoesNormalizadas = colunasGridCotacoes.map((coluna, indice) => {
    ...
    return {
      ...coluna,
      base: TOTAL_COLUNAS_GRID_COTACOES,
      ...
      span: normalizarSpanColuna(
        ...
        spanPadraoColuna,
        spanFixoColuna,
        normalizarBaseConfiguracao(configuracao?.base, TOTAL_COLUNAS_GRID_COTACOES)
      )
    };
  }).map((coluna) => ({
    ...coluna,
  ...
  return reordenarConfiguracoesColunasGridCotacoes(configuracoesNormalizadas);
}

export function normalizarColunasGridCotacoes(valor) {
  return normalizarConfiguracoesColunasGridCotacoes(valor)
    .filter((coluna) => coluna.visivel)
    .sort(ordenarColunasGridCotacoes);
}

export function reordenarConfiguracoesColunasGridCotacoes(configuracoes) {
...
export function reposicionarConfiguracaoColunaGridCotacoes(configuracoes, idColuna, ordemDesejada) {
  const lista = Array.isArray(configuracoes) ? configuracoes.map((coluna) => ({ ...coluna })) : [];
  ...
  if (idColuna === 'acoes') {
    return reordenarConfiguracoesColunasGridCotacoes(lista);
  }
...
function ordenarColunasGridCotacoes(colunaA, colunaB) {
...
function normalizarSpanColuna(valor, valorPadrao = 1, valorFixo = null, baseConfiguracao = TOTAL_COLUNAS_GRID_COTACOES) {
  if (Number.isFinite(Number(valorFixo)) && Number(valorFixo) > 0) {
    return Math.min(TOTAL_COLUNAS_GRID_COTACOES, Math.max(1, Number(valorFixo)));
  }
...
function normalizarRotuloColuna(valor, valorPadrao = '') {
  ...
  const textoNormalizado = mapaRotulosLegadosColunasGridCotacoes.get(texto) || texto;
  return textoNormalizado || String(valorPadrao || '').trim();
}

function normalizarBaseConfiguracao(valor, valorPadrao = BASE_LEGADA_COLUNAS_GRID_COTACOES) {
  const numero = Number.parseInt(String(valor ?? '').trim(), 10);
  return Number.isFinite(numero) && numero > 0 ? numero : valorPadrao;
}
...
function normalizarSpanDefinicaoColuna(valor) {
  ...
  return converterSpanParaBaseAtual(Number(valor), BASE_LEGADA_COLUNAS_GRID_COTACOES);
}

function converterSpanParaBaseAtual(valor, baseOrigem = TOTAL_COLUNAS_GRID_COTACOES) {
  const numero = normalizarNumeroInteiro(valor, 1);

  if (baseOrigem === TOTAL_COLUNAS_GRID_COTACOES) {
    return Math.min(TOTAL_COLUNAS_GRID_COTACOES, Math.max(1, numero));
  }

  return Math.min(
    TOTAL_COLUNAS_GRID_COTACOES,
    Math.max(1, Math.floor((numero * TOTAL_COLUNAS_GRID_COTACOES) / baseOrigem))
  );
}
