...
export function normalizarConfiguracoesColunasGridPedidos(valor) {
  ...
  const configuracoesNormalizadas = colunasGridPedidos.map((coluna, indice) => {
    ...
    return {
      ...coluna,
      base: TOTAL_COLUNAS_GRID_PEDIDOS,
...
function normalizarSpanColuna(valor, valorPadrao = 1, valorFixo = null, baseConfiguracao = TOTAL_COLUNAS_GRID_PEDIDOS) {
  if (Number.isFinite(Number(valorFixo)) && Number(valorFixo) > 0) {
    return Math.min(TOTAL_COLUNAS_GRID_PEDIDOS, Math.max(1, Number(valorFixo)));
  }
...
function normalizarSpanDefinicaoColuna(valor) {
  ...
  return converterSpanParaBaseAtual(Number(valor), BASE_LEGADA_COLUNAS_GRID_PEDIDOS);
}

function converterSpanParaBaseAtual(valor, baseOrigem = TOTAL_COLUNAS_GRID_PEDIDOS) {
  const numero = normalizarNumeroInteiro(valor, 1);

  if (baseOrigem === TOTAL_COLUNAS_GRID_PEDIDOS) {
