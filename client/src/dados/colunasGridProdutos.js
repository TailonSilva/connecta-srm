...
const idsPermitidos = new Set(colunasGridProdutos.map((coluna) => coluna.id));
...
export function normalizarColunasGridProdutos(valor) {
  return normalizarConfiguracoesColunasGridProdutos(valor)
    .filter((coluna) => coluna.visivel)
    .sort(ordenarColunasGridProdutos);
}

export function reordenarConfiguracoesColunasGridProdutos(configuracoes) {
