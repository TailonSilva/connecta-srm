export const TOTAL_COLUNAS_GRID_FORNECEDORES = 100;
const BASE_LEGADA_COLUNAS_GRID_FORNECEDORES = 24;
const MAX_SPAN_COLUNA_OCULTA = 19;

export const colunasGridFornecedores = [
  {
    id: 'imagem',
    rotulo: 'Imagem',
...
const idsPermitidos = new Set(colunasGridFornecedores.map((coluna) => coluna.id));
const mapaColunasGridFornecedores = new Map(
  colunasGridFornecedores.map((coluna) => [coluna.id, coluna])
);

export function normalizarConfiguracoesColunasGridFornecedores(valor) {
  ...
  listaNormalizada.forEach((item, indice) => {
    ...
    const colunaBase = mapaColunasGridFornecedores.get(id);
  ...
  const configuracoesNormalizadas = colunasGridFornecedores.map((coluna, indice) => {
    ...
    return {
      ...coluna,
      base: TOTAL_COLUNAS_GRID_FORNECEDORES,
      ...
      span: normalizarSpanColuna(
        ...
        spanPadraoColuna,
        spanFixoColuna,
        normalizarBaseConfiguracao(configuracao?.base, TOTAL_COLUNAS_GRID_FORNECEDORES)
      )
    };
  }).map((coluna) => ({
    ...coluna,
  ...
  return reordenarConfiguracoesColunasGridFornecedores(configuracoesNormalizadas);
}

export function normalizarColunasGridFornecedores(valor) {
  return normalizarConfiguracoesColunasGridFornecedores(valor)
    .filter((coluna) => coluna.visivel)
    .sort(ordenarColunasGridFornecedores);
}

export function reordenarConfiguracoesColunasGridFornecedores(configuracoes) {
...
export function reposicionarConfiguracaoColunaGridFornecedores(configuracoes, idColuna, ordemDesejada) {
  const lista = Array.isArray(configuracoes) ? configuracoes.map((coluna) => ({ ...coluna })) : [];
  ...
  if (idColuna === 'acoes') {
    return reordenarConfiguracoesColunasGridFornecedores(lista);
  }
...
function ordenarColunasGridFornecedores(colunaA, colunaB) {
...
function normalizarSpanColuna(valor, valorPadrao = 1, valorFixo = null, baseConfiguracao = TOTAL_COLUNAS_GRID_FORNECEDORES) {
  if (Number.isFinite(Number(valorFixo)) && Number(valorFixo) > 0) {
    return Math.min(TOTAL_COLUNAS_GRID_FORNECEDORES, Math.max(1, Number(valorFixo)));
  }
...
function normalizarBaseConfiguracao(valor, valorPadrao = BASE_LEGADA_COLUNAS_GRID_FORNECEDORES) {
  const numero = Number.parseInt(String(valor ?? '').trim(), 10);
  return Number.isFinite(numero) && numero > 0 ? numero : valorPadrao;
}
...
function normalizarSpanDefinicaoColuna(valor) {
  ...
  return converterSpanParaBaseAtual(Number(valor), BASE_LEGADA_COLUNAS_GRID_FORNECEDORES);
}

function converterSpanParaBaseAtual(valor, baseOrigem = TOTAL_COLUNAS_GRID_FORNECEDORES) {
  const numero = normalizarNumeroInteiro(valor, 1);

  if (baseOrigem === TOTAL_COLUNAS_GRID_FORNECEDORES) {
    return Math.min(TOTAL_COLUNAS_GRID_FORNECEDORES, Math.max(1, numero));
  }

  return Math.min(
    TOTAL_COLUNAS_GRID_FORNECEDORES,
    Math.max(1, Math.floor((numero * TOTAL_COLUNAS_GRID_FORNECEDORES) / baseOrigem))
  );
}
