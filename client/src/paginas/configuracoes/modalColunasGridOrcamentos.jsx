...
export function ModalColunasGridOrcamentos({
  ...
  const colunasOrdenadas = useMemo(
    () => reordenarConfiguracoesColunasGridOrcamentos(configuracoesColunas),
  ...
  function atualizarVisibilidade(idColuna, visivel) {
    ...
    definirConfiguracoesColunas((estadoAtual) => reordenarConfiguracoesColunasGridOrcamentos(
  ...
  function salvarEdicaoColuna(evento) {
    ...
    definirConfiguracoesColunas((estadoAtual) => {
      ...
      if (!colunaAtualizada?.visivel && !colunaAtualizada?.obrigatoria) {
        return reordenarConfiguracoesColunasGridOrcamentos(proximaLista);
      }

      return reposicionarConfiguracaoColunaGridOrcamentos(
        proximaLista,
        colunaEmEdicao,
  ...
  async function submeterFormulario(evento) {
    ...
    const colunasNormalizadas = reordenarConfiguracoesColunasGridOrcamentos(configuracoesColunas);
    ...
    if (totalSpan > TOTAL_COLUNAS_GRID_ORCAMENTOS) {
      definirMensagemErro(
        `As colunas visiveis somam ${totalSpan} espacos. Reduza para no maximo ${TOTAL_COLUNAS_GRID_ORCAMENTOS}.`
      );
      return;
    }
    ...
    try {
      ...
      definirMensagemErro(erro.message || 'Nao foi possivel salvar as colunas do grid de orcamentos.');
      definirSalvando(false);
    }
  }
    ...
    <div className="camadaModal" role="presentation" onMouseDown={fecharAoClicarNoFundo}>
        ...
        <div className="corpoModalCliente">
          <section className="gradeCamposModalCliente">
            <div className="campoFormularioIntegral">
              ...
              <p className={`resumoColunasGridAtendimentos ${totalColunasUtilizadas > TOTAL_COLUNAS_GRID_ORCAMENTOS ? 'excedido' : ''}`}>
                Espaco usado pelas colunas visiveis: {totalColunasUtilizadas}/{TOTAL_COLUNAS_GRID_ORCAMENTOS}
              </p>
            </div>
      ...
      {colunaEmEdicao ? (
        <div className="camadaModalContato camadaModalFiltroPeriodo" role="presentation" onMouseDown={fecharModalEdicao}>
          <form className="modalContatoCliente modalFiltros modalEdicaoColunaGridAtendimentos" role="dialog" aria-modal="true" aria-labelledby="tituloModalEdicaoColunaGridOrcamentos" onMouseDown={(evento) => evento.stopPropagation()} onSubmit={salvarEdicaoColuna}>
            <header className="cabecalhoModalContato">
              <h3 id="tituloModalEdicaoColunaGridOrcamentos">Editar {obterRotuloColuna(colunasOrdenadas, colunaEmEdicao)}</h3>
