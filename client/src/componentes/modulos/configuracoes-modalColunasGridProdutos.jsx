...
import {
  normalizarConfiguracoesColunasGridProdutos,
...
export function ModalColunasGridProdutos({
  ...
  useEffect(() => {
    ...
    definirConfiguracoesColunas(
      normalizarConfiguracoesColunasGridProdutos(empresa?.colunasGridProdutos)
    );
  ...
  const colunasOrdenadas = useMemo(
    () => reordenarConfiguracoesColunasGridProdutos(configuracoesColunas),
  ...
  function atualizarVisibilidade(idColuna, visivel) {
    ...
    definirConfiguracoesColunas((estadoAtual) => reordenarConfiguracoesColunasGridProdutos(
  ...
  function salvarEdicaoColuna(evento) {
    ...
    definirConfiguracoesColunas((estadoAtual) => {
      ...
      if (!colunaAtualizada?.visivel && !colunaAtualizada?.obrigatoria) {
        return reordenarConfiguracoesColunasGridProdutos(proximaLista);
      }

      return reposicionarConfiguracaoColunaGridProdutos(
        proximaLista,
        colunaEmEdicao,
  ...
  async function submeterFormulario(evento) {
    ...
    const colunasNormalizadas = reordenarConfiguracoesColunasGridProdutos(configuracoesColunas);
    ...
    if (totalSpan > TOTAL_COLUNAS_GRID_PRODUTOS) {
      definirMensagemErro(
        `As colunas visiveis somam ${totalSpan} espacos. Reduza para no maximo ${TOTAL_COLUNAS_GRID_PRODUTOS}.`
      );
      return;
    }
    ...
    try {
      await aoSalvar({
        colunasGridProdutos: colunasNormalizadas.map((coluna) => ({
          id: coluna.id,
          base: coluna.base,
      ...
      definirMensagemErro(erro.message || 'Nao foi possivel salvar as colunas do grid de produtos.');
      definirSalvando(false);
    }
  }
        ...
        <div className="corpoModalFornecedor">
          <section className="gradeCamposModalFornecedor">
            <div className="campoFormularioIntegral">
              ...
              <p className={`resumoColunasGridAtendimentos ${totalColunasUtilizadas > TOTAL_COLUNAS_GRID_PRODUTOS ? 'excedido' : ''}`}>
                Espaco usado pelas colunas visiveis: {totalColunasUtilizadas}/{TOTAL_COLUNAS_GRID_PRODUTOS}
              </p>
            </div>
        ...
        <div className="camadaModalContato camadaModalFiltroPeriodo" role="presentation" onMouseDown={fecharModalEdicao}>
          ...
          >
            <header className="cabecalhoModalContato">
              <h3 id="tituloModalEdicaoColunaGridProdutos">
...
function obterRotuloConfiguracaoColuna(coluna) {
  ...
  if (coluna.id === 'idProduto') {
    return 'Codigo Interno do Produto';
  }
  ...
  if (coluna.id === 'grupo') {
    return 'Grupo do Produto';
  }

  if (coluna.id === 'idGrupo') {
    return 'Grupo do Cadastro';
  }
...
function normalizarSpan(valor, valorPadrao = 1) {
  return Math.min(TOTAL_COLUNAS_GRID_PRODUTOS, normalizarNumeroInteiro(valor, valorPadrao));
}
