...
import {
  normalizarConfiguracoesColunasGridFornecedores,
  reposicionarConfiguracaoColunaGridFornecedores,
  reordenarConfiguracoesColunasGridFornecedores,
  TOTAL_COLUNAS_GRID_FORNECEDORES
} from '../../dados/colunasGridFornecedores';

export function ModalColunasGridFornecedores({
  aberto,
  empresa,
  ...
  useEffect(() => {
    ...
    definirConfiguracoesColunas(
      normalizarConfiguracoesColunasGridFornecedores(empresa?.colunasGridFornecedores)
    );
  ...
  const colunasOrdenadas = useMemo(
    () => reordenarConfiguracoesColunasGridFornecedores(configuracoesColunas),
  ...
  function atualizarVisibilidade(idColuna, visivel) {
    ...
    definirConfiguracoesColunas((estadoAtual) => reordenarConfiguracoesColunasGridFornecedores(
  ...
  function salvarEdicaoColuna(evento) {
    ...
    definirConfiguracoesColunas((estadoAtual) => {
      ...
      if (!colunaAtualizada?.visivel && !colunaAtualizada?.obrigatoria) {
        return reordenarConfiguracoesColunasGridFornecedores(proximaLista);
      }

      return reposicionarConfiguracaoColunaGridFornecedores(
        proximaLista,
        colunaEmEdicao,
  ...
  async function submeterFormulario(evento) {
    ...
    const colunasNormalizadas = reordenarConfiguracoesColunasGridFornecedores(configuracoesColunas);
    ...
    if (totalSpan > TOTAL_COLUNAS_GRID_FORNECEDORES) {
      definirMensagemErro(
        `As colunas visiveis somam ${totalSpan} espacos. Reduza para no maximo ${TOTAL_COLUNAS_GRID_FORNECEDORES}.`
      );
      return;
    }
    ...
    try {
      await aoSalvar({
        colunasGridFornecedores: colunasNormalizadas.map((coluna) => ({
          id: coluna.id,
          base: coluna.base,
  ...
  return (
    <div className="camadaModal" role="presentation" onMouseDown={fecharAoClicarNoFundo}>
      <form
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="tituloModalColunasGridFornecedores"
        onMouseDown={(evento) => evento.stopPropagation()}
        onSubmit={submeterFormulario}
      >
        <header className="cabecalhoModalFornecedor">
          <h2 id="tituloModalColunasGridFornecedores">Colunas do Grid de Fornecedores</h2>
        ...
        <div className="corpoModalFornecedor">
          <section className="gradeCamposModalFornecedor">
            <div className="campoFormularioIntegral">
              ...
              <p className={`resumoColunasGridAtendimentos ${totalColunasUtilizadas > TOTAL_COLUNAS_GRID_FORNECEDORES ? 'excedido' : ''}`}>
                Espaco usado pelas colunas visiveis: {totalColunasUtilizadas}/{TOTAL_COLUNAS_GRID_FORNECEDORES}
              </p>
            </div>
      ...
      {colunaEmEdicao ? (
        <div className="camadaModalContato camadaModalFiltroPeriodo" role="presentation" onMouseDown={fecharModalEdicao}>
          <form
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="tituloModalEdicaoColunaGridFornecedores"
            onMouseDown={(evento) => evento.stopPropagation()}
            onSubmit={salvarEdicaoColuna}
          >
            <header className="cabecalhoModalContato">
              <h3 id="tituloModalEdicaoColunaGridFornecedores">
              ...
              <div className="painelEdicaoColunaGridAtendimentos">
                ...
                <label className="campoEdicaoColunaGridAtendimentos">
                  ...
                  <input
                    type="number"
                    min="1"
                    max={String(TOTAL_COLUNAS_GRID_FORNECEDORES)}
...
function obterRotuloConfiguracaoColuna(coluna) {
  ...
  if (coluna.id === 'comprador' || coluna.id === 'idComprador') {
...
function normalizarSpan(valor, valorPadrao = 1) {
  return Math.min(TOTAL_COLUNAS_GRID_FORNECEDORES, normalizarNumeroInteiro(valor, valorPadrao));
}
