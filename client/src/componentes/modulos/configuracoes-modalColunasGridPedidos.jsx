...
export function ModalColunasGridPedidos({
  ...
  const colunasOrdenadas = useMemo(
    () => reordenarConfiguracoesColunasGridPedidos(configuracoesColunas),
  ...
  function atualizarVisibilidade(idColuna, visivel) {
    ...
    definirConfiguracoesColunas((estadoAtual) => reordenarConfiguracoesColunasGridPedidos(
  ...
  function salvarEdicaoColuna(evento) {
    ...
    definirConfiguracoesColunas((estadoAtual) => {
      ...
      if (!colunaAtualizada?.visivel && !colunaAtualizada?.obrigatoria) {
        return reordenarConfiguracoesColunasGridPedidos(proximaLista);
      }

      return reposicionarConfiguracaoColunaGridPedidos(
        proximaLista,
        colunaEmEdicao,
  ...
  async function submeterFormulario(evento) {
    ...
    const colunasNormalizadas = reordenarConfiguracoesColunasGridPedidos(configuracoesColunas);
    ...
    if (totalSpan > TOTAL_COLUNAS_GRID_PEDIDOS) {
      definirMensagemErro(
        `As colunas visiveis somam ${totalSpan} espacos. Reduza para no maximo ${TOTAL_COLUNAS_GRID_PEDIDOS}.`
      );
      return;
    }
  ...
  return (
    <div className="camadaModal" role="presentation" onMouseDown={fecharAoClicarNoFundo}>
      <form
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="tituloModalColunasGridPedidos"
        ...
        <div className="corpoModalCliente">
          <section className="gradeCamposModalCliente">
            <div className="campoFormularioIntegral">
              ...
              <p className={`resumoColunasGridAtendimentos ${totalColunasUtilizadas > TOTAL_COLUNAS_GRID_PEDIDOS ? 'excedido' : ''}`}>
                Espaco usado pelas colunas visiveis: {totalColunasUtilizadas}/{TOTAL_COLUNAS_GRID_PEDIDOS}
              </p>
            </div>
      ...
      {colunaEmEdicao ? (
        <div className="camadaModalContato camadaModalFiltroPeriodo" role="presentation" onMouseDown={fecharModalEdicao}>
          <form
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="tituloModalEdicaoColunaGridPedidos"
            onMouseDown={(evento) => evento.stopPropagation()}
            onSubmit={salvarEdicaoColuna}
          >
            <header className="cabecalhoModalContato">
              <h3 id="tituloModalEdicaoColunaGridPedidos">
              ...
              <div className="painelEdicaoColunaGridAtendimentos">
                ...
                <label className="campoEdicaoColunaGridAtendimentos">
                  ...
                  <input
                    type="number"
                    min="1"
                    max={String(TOTAL_COLUNAS_GRID_PEDIDOS)}
