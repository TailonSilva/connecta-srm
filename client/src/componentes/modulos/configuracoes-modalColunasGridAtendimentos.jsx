...
export function ModalColunasGridAtendimentos({
  ...
  useEffect(() => {
    ...
    definirConfiguracoesColunas(
      normalizarConfiguracoesColunasGridAtendimentos(empresa?.colunasGridAtendimentos)
    );
  ...
  const colunasOrdenadas = useMemo(
    () => reordenarConfiguracoesColunasGridAtendimentos(configuracoesColunas),
  ...
  function atualizarVisibilidade(idColuna, visivel) {
    ...
    definirConfiguracoesColunas((estadoAtual) => reordenarConfiguracoesColunasGridAtendimentos(
  ...
  function salvarEdicaoColuna(evento) {
    ...
    definirConfiguracoesColunas((estadoAtual) => {
      ...
      if (!colunaAtualizada?.visivel && !colunaAtualizada?.obrigatoria) {
        return reordenarConfiguracoesColunasGridAtendimentos(proximaLista);
      }

      return reposicionarConfiguracaoColunaGridAtendimentos(
        proximaLista,
        colunaEmEdicao,
  ...
  async function submeterFormulario(evento) {
    ...
    const colunasNormalizadas = reordenarConfiguracoesColunasGridAtendimentos(configuracoesColunas);
    ...
    if (totalSpan > TOTAL_COLUNAS_GRID_ATENDIMENTOS) {
      definirMensagemErro(
        `As colunas visiveis somam ${totalSpan} espacos. Reduza para no maximo ${TOTAL_COLUNAS_GRID_ATENDIMENTOS}.`
      );
      return;
    }
    ...
    try {
      await aoSalvar({
        colunasGridAtendimentos: colunasNormalizadas.map((coluna) => ({
          id: coluna.id,
          base: coluna.base,
      ...
      definirMensagemErro(erro.message || 'Nao foi possivel salvar as colunas do grid de atendimentos.');
      definirSalvando(false);
    }
  }
        ...
        <div className="corpoModalFornecedor">
          <section className="gradeCamposModalFornecedor">
            <div className="campoFormularioIntegral">
              ...
              <p className={`resumoColunasGridAtendimentos ${totalColunasUtilizadas > TOTAL_COLUNAS_GRID_ATENDIMENTOS ? 'excedido' : ''}`}>
                Espaco usado pelas colunas visiveis: {totalColunasUtilizadas}/{TOTAL_COLUNAS_GRID_ATENDIMENTOS}
              </p>
            </div>

            <div className="campoFormularioIntegral listaCheckboxesGridAtendimentos">
              {colunasOrdenadas.map((coluna) => {
                const rotuloConfiguracao = obterRotuloConfiguracaoColuna(coluna);
                const chipOrdem = coluna.visivel || coluna.obrigatoria
                  ? `Ordem ${coluna.ordem}`
                  : 'Sem ordem';
                ...
                return (
                  ...
                  >
                    <label className="cabecalhoItemCheckboxGridAtendimentos">
                      ...
                      />
                      <span>{rotuloConfiguracao}</span>
                    </label>

                    <div className="acoesItemCheckboxGridAtendimentos">
                      <div className="grupoChipsGridAtendimentos">
                        {coluna.obrigatoria ? (
                          <span className="chipResumoGridAtendimentos chipResumoGridAtendimentosObrigatorio">
                            Obrigatoria
                          </span>
                        ) : null}
                        <span className="chipResumoGridAtendimentos chipResumoGridAtendimentosRotulo" title={chipRotulo}>
                          {chipRotulo}
                        </span>
                        <span className={`chipResumoGridAtendimentos ${coluna.visivel ? 'ativo' : 'inativo'}`}>
                          {chipOrdem}
                        </span>
                      ...
                      <BotaoAcaoGrade
                        icone="editar"
                        titulo={`Editar ${rotuloConfiguracao}`}
        ...
        <div className="camadaModalContato camadaModalFiltroPeriodo" role="presentation" onMouseDown={fecharModalEdicao}>
          ...
          >
            <header className="cabecalhoModalContato">
              <h3 id="tituloModalEdicaoColunaGridAtendimentos">
...
function obterRotuloConfiguracaoColuna(coluna) {
  ...
  if (coluna.id === 'horaFim') {
    return 'Horario de Fim';
  }

  if (coluna.id === 'agendamento') {
    return 'Numero do Agendamento Vinculado';
  }
...
function normalizarSpan(valor, valorPadrao = 1) {
  return Math.min(TOTAL_COLUNAS_GRID_ATENDIMENTOS, normalizarNumeroInteiro(valor, valorPadrao));
}
