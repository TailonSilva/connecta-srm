...
import {
  normalizarConfiguracoesColunasGridOrdensCompra,
  reposicionarConfiguracaoColunaGridOrdensCompra,
  reordenarConfiguracoesColunasGridOrdensCompra,
  TOTAL_COLUNAS_GRID_ORDENS_COMPRA
} from '../../dados/colunasGridOrdensCompra';

export function ModalColunasGridOrdensCompra({
  aberto,
  empresa,
  ...
  useEffect(() => {
    ...
    definirConfiguracoesColunas(
      normalizarConfiguracoesColunasGridOrdensCompra(empresa?.colunasGridOrdensCompra)
    );
  ...
  const colunasOrdenadas = useMemo(
    () => reordenarConfiguracoesColunasGridOrdensCompra(configuracoesColunas),
  ...
  function atualizarVisibilidade(idColuna, visivel) {
    ...
    definirConfiguracoesColunas((estadoAtual) => reordenarConfiguracoesColunasGridOrdensCompra(
  ...
  function salvarEdicaoColuna(evento) {
    ...
    definirConfiguracoesColunas((estadoAtual) => {
      ...
      if (!colunaAtualizada?.visivel && !colunaAtualizada?.obrigatoria) {
        return reordenarConfiguracoesColunasGridOrdensCompra(proximaLista);
      }

      return reposicionarConfiguracaoColunaGridOrdensCompra(
        proximaLista,
        colunaEmEdicao,
  ...
  async function submeterFormulario(evento) {
    ...
    const colunasNormalizadas = reordenarConfiguracoesColunasGridOrdensCompra(configuracoesColunas);
    ...
    if (totalSpan > TOTAL_COLUNAS_GRID_ORDENS_COMPRA) {
      definirMensagemErro(
        `As colunas visiveis somam ${totalSpan} espacos. Reduza para no maximo ${TOTAL_COLUNAS_GRID_ORDENS_COMPRA}.`
      );
      return;
    }
    ...
    try {
      await aoSalvar({
        colunasGridOrdensCompra: colunasNormalizadas.map((coluna) => ({
          id: coluna.id,
          base: coluna.base,
  ...
  return (
    <div className="camadaModal" role="presentation" onMouseDown={fecharAoClicarNoFundo}>
      <form
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="tituloModalColunasGridOrdensCompra"
        onMouseDown={(evento) => evento.stopPropagation()}
        onSubmit={submeterFormulario}
      >
        <header className="cabecalhoModalFornecedor">
          <h2 id="tituloModalColunasGridOrdensCompra">Colunas do Grid de Ordens de Compra</h2>
        ...
        <div className="corpoModalFornecedor">
          <section className="gradeCamposModalFornecedor">
            <div className="campoFormularioIntegral">
              ...
              <p className={`resumoColunasGridAtendimentos ${totalColunasUtilizadas > TOTAL_COLUNAS_GRID_ORDENS_COMPRA ? 'excedido' : ''}`}>
                Espaco usado pelas colunas visiveis: {totalColunasUtilizadas}/{TOTAL_COLUNAS_GRID_ORDENS_COMPRA}
              </p>
            </div>
      ...
      {colunaEmEdicao ? (
        <div className="camadaModalContato camadaModalFiltroPeriodo" role="presentation" onMouseDown={fecharModalEdicao}>
          <form
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="tituloModalEdicaoColunaGridOrdensCompra"
            onMouseDown={(evento) => evento.stopPropagation()}
            onSubmit={salvarEdicaoColuna}
          >
            <header className="cabecalhoModalContato">
              <h3 id="tituloModalEdicaoColunaGridOrdensCompra">
              ...
              <div className="painelEdicaoColunaGridAtendimentos">
                ...
                <label className="campoEdicaoColunaGridAtendimentos">
                  ...
                  <input
                    type="number"
                    min="1"
                    max={String(TOTAL_COLUNAS_GRID_ORDENS_COMPRA)}
...
function obterRotuloConfiguracaoColuna(coluna) {
  ...
  if (coluna.id === 'idCotacao' || coluna.id === 'codigoCotacaoOrigem') {
...
function normalizarSpan(valor, valorPadrao = 1) {
  return Math.min(TOTAL_COLUNAS_GRID_ORDENS_COMPRA, normalizarNumeroInteiro(valor, valorPadrao));
}
